...
-- Semua user yang jadi member bisa baca proyek
CREATE POLICY "Member baca proyek"
  ON proyek FOR SELECT
  USING (
    EXISTS (SELECT 1 FROM proyek_members WHERE proyek_id = proyek.id AND user_id = auth.uid())
...
CREATE TABLE IF NOT EXISTS proyek_members (
  ...
  user_id     uuid REFERENCES auth.users ON DELETE CASCADE,
  role        text DEFAULT 'marketing', -- 'admin' | 'marketing'
...
-- Tambah kolom proyek_id ke tabel konsumen
ALTER TABLE konsumen ADD COLUMN IF NOT EXISTS proyek_id uuid REFERENCES proyek ON DELETE SET NULL;
...
-- Update RLS konsumen: filter juga by proyek_id
-- (Policies yang ada sudah cukup karena kita filter di aplikasi)
